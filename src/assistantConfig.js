...
- warmTransfer: Use this to brief the front desk via SMS about guest requests or escalations. Always pass callId as the live conversationId (use {{conversationId}}—never room numbers or other placeholders), along with guest status (Prospect or In-house + identifiers), summary, action items, mood, and transfer reason. Include connectCall: "bridge" only if the guest explicitly asks to speak with the front desk or the matter demands a live handoff; otherwise omit it and keep assisting the caller yourself.
